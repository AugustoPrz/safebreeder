...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex">
      ...
      {/* Mobile bottom nav: Establecimientos · Scan (center, destacado) · Dashboard */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-30 bg-surface border-t border-border">
        <div className="grid grid-cols-3 h-16 items-stretch">
          <Link
            href="/establishments"
            className={`flex flex-col items-center justify-center gap-1 text-xs font-medium ${
              isActive("/establishments") ? "text-primary" : "text-text-muted"
            }`}
          >
            <EstablishmentsIcon className="w-5 h-5" />
...
function ScanIcon(props: { className?: string }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
